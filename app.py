...
# 🎨 1. 웹 페이지 기본 세팅
# ==========================================
st.set_page_config(page_title="하나투어 캠페인 Ops Automation", page_icon="✈️", layout="wide")
...
def gather_deep_sns_data(keywords_str, uploaded_file):
    ...
    # 🟢 네이버: 본문 텍스트(description) 부활 & 수집량 20개
...
# 🎛️ 4. 왼쪽 사이드바 메뉴 및 설정
# ==========================================
st.sidebar.markdown("## 🧭 캠페인 자동화 메뉴")
st.sidebar.markdown("원하시는 업무 탭을 선택하세요.")

# 👉 [핵심] 큼직하고 명확한 라디오 버튼으로 메뉴 순서 및 이름 완벽 교체!
page_selection = st.sidebar.radio(
    "메뉴 선택", 
    ["📈 1. 지역별 마케팅 인사이트", "🛍️ 2. 여행상품기획 인사이트"],
    label_visibility="collapsed" # 기본 작은 라벨은 숨겨서 깔끔하게 만듭니다.
)

st.sidebar.markdown("---")

# 👉 [핵심] 엑셀 업로드 창을 클릭 시 열리는 아코디언(Expander) 형태로 변경
with st.sidebar.expander("📸 인스타그램 엑셀 수동 업로드 (클릭하여 열기)", expanded=False):
    st.info("Apify에서 추출한 `.xlsx` 파일을 올려주시면 심층 분석에 반영됩니다.")
...
if page_selection == "📈 1. 지역별 마케팅 인사이트":
    ...
    st.caption("프로모션 후보 지역으로 선정된 곳의 SNS 트렌드를 심층 크롤링하여 페르소나와 전시/메시지 가이드를 추출합니다.")
    
    # 👉 [핵심] 로직 및 기준 안내 아코디언 상단 추가
        ...
        with st.spinner('본문 데이터까지 깊게 파고들어 SNS 트렌드를 수집 중입니다 (약 1~2분 소요)...'):
            ...
            try:
                ...
                * (가장 반응이 뜨거운 키워드 5개와, 이를 활용한 광고 카피 아이디어)
                ## 3. 🛍️ 타겟 맞춤 상품 전시 가이드 
                * (하나투어 기획전 페이지나 상품 구성을 어떻게 짜야 할지 구체적 가이드 제시)
                ## 4. 📸 트렌드 레퍼런스 비주얼
...
# ------------------------------------------
    ...
    if st.button("✨ 세부 상품 기획안 생성", type="primary"):
        if not selected_types:
            ...
            with st.spinner('SNS 데이터를 긁어모으고 있습니다...'):
            ...
            with st.spinner('세부 기획안을 작성 중입니다...'):
                try:
                    client = genai.Client(api_key=GEMINI_API_KEY)
                    prompt_prod = f"""
                    당신은 여행 상품 기획자입니다. 아젠다: "{agenda_input_prod}"
