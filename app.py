...
# 💡 [핵심 변경] 데이터를 H, I, J열에 나눠서 담는 저장 함수
def save_to_gsheet(agenda, keywords, product_types, result, country, city, n_raw, y_raw, i_raw):
    try:
        ...
        # 각 셀당 안전하게 35,000자씩 컷 (구글 시트 전체 안정성 확보)
        new_row = [
...
# 💡 [핵심 변경] 데이터를 3개의 덩어리로 분리해서 리턴하는 딥 크롤링 엔진
    ...
    if uploaded_file:
        try:
            ...
            for _, row in df.head(10).iterrows():
                i_out += f"[인스타-엑셀] 본문:{str(row.get('text',''))[:2000]}\n"
        except: pass
    else:
        try:
            ...
            for item in api_c.dataset(run["defaultDatasetId"]).iterate_items():
                i_out += f"[인스타-자동] 본문:{str(item.get('text',''))[:2000]}\n"
        except: pass
...
if page_selection == "📈 1. 지역별 마케팅 인사이트":
    ...
    st.caption("SNS 트렌드를 심층 크롤링하여 페르소나와 메시지 가이드를 추출합니다.")
    
    with st.expander("ℹ️ 하이브리드 RAG 수집 로직 안내", expanded=True):
        st.markdown("* **네이버:** 상위 5개 본문 전체 스틸  \n* **유튜브:** 제목+설명+음성 자막(대본) 스틸  \n* **인스타:** 본문 및 핵심 해시태그 전체")
